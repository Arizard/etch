...
function getTripIcon(str) {
  return svgIcons[Math.floor(hashToFloat(str) * svgIcons.length)];
}
...
export default function Reply({
  ...
  const tripIcon = signature ? getTripIcon(signature) : svgAnon;
  const tripcodeDisplay =
    signature && signature !== '' ? `#${signature.slice(0, 15)}` : '';

  return (
    <div className="reply-card-container">
      <div className="reply-card-byline">
        <div className={`reply-card-avatar ${tripColorClass}`}>
